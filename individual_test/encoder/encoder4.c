...
int main(int argc, char * argv[])
{
   int pos = 125;
   long min, max;
   long gpiodelay_value = 150;		// was 50
   ...
   const char *selem_name = "Mic1 Boost";
...
//   pullUpDnControl (ENCODER_A, PUD_UP);
   ...
   // Get current volume
   if (x = snd_mixer_selem_get_capture_volume (elem, SND_MIXER_SCHN_FRONT_LEFT, &currentVolume))
   {
        printf("%d %s\n", x, snd_strerror(x));
   }
   else if (DEBUG_PRINT) printf("Current ALSA volume LEFT: %ld\n", currentVolume);

   if (x = snd_mixer_selem_get_capture_volume (elem, SND_MIXER_SCHN_FRONT_RIGHT, &currentVolume))
   {
        printf("%d %s\n", x, snd_strerror(x));
   }
      ...
      if (encoderPos != pos)
      {
	      ...
	      else if (encoderPos < pos)
	      {
			 ...
			 if (currentVolume < min) currentVolume = 0;
              ...
              if (x = snd_mixer_selem_set_capture_volume_all(elem, currentVolume))
              {
                     printf(" ERROR %d %s\n", x, snd_strerror(x));
